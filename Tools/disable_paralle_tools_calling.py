...
agent = create_react_agent(
    model=llm_with_fallback.bind_tools(tools=[addition, multiply], parallel_tool_calls=False),
    tools=[addition, multiply],
)

respnse = agent.invoke({"messages": [{"role": "user", "content": "what's 3 + 5 and 4 * 7?"}]})
print('**' * 50)
print('--' * 20)
print('User Query')
